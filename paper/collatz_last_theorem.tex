...
\begin{table}[h]
\centering
\caption{Deviation exponent $|f_P - \text{ref}| \sim n^b$. The corrected null model (each round = one P-step followed by a geometrically distributed cluster of N-steps; no loose N-steps) reproduces the map's structure exactly.}
\begin{tabular}{lcc}
\toprule
System & Reference & Exponent $b$ \\
\midrule
CLT naive (i.i.d.\ Bernoulli) & $1/2$ & $-0.500$ \\
\textbf{Null (matched clusters, corrected)} & $\bar\mu_{\text{null}} = 0.333$ & $\mathbf{-0.489}$ \\
Collatz & $\mu = 0.324$ & $-0.264$ \\
\bottomrule
\end{tabular}
\end{table}

The corrected null cancels deviation at the CLT rate ($-0.489 \approx -0.5$) --- the chance process with the map's own structure behaves exactly as theory predicts. Collatz cancels \emph{more slowly} ($-0.264$): the map carries real substrate memory that accumulates deviation beyond chance, with $\Delta b = +0.22$.

The ceilings confirm it: Collatz $C^* = 0.265$ vs null $C^* = 0.129$ --- \emph{double}. The excess is substrate, not noise. A divergent orbit would have to live in this memory.

The earlier version of this analysis used a mis-specified null (with loose N-steps, producing $f_P = 0.2$ instead of $1/3$); that null biased the contrast and briefly made Collatz appear ``inside chance''. The corrected null restores the conclusion: the excess is real, confirmed, and bounded.
...
\begin{table}[h]
\centering
\caption{The final picture (corrected null, 100k orbits).}
\begin{tabular}{lccc}
\toprule
Quantity & Collatz & Null (corrected) & Interpretation \\
\midrule
$\mu$ (invariant measure) & 0.324 & 0.333 & both near 1/3 \\
Exponent $b$ & $-0.264$ & $-0.489$ & Collatz cancels slower \\
Gap & $+0.225$ & --- & \textbf{real substrate memory} \\
Ceiling $C^*$ & 0.265 & 0.129 & \textbf{double the chance} \\
$f_P^* - \mu$ (margin) & 0.383 & --- & \\
\textbf{Bound margin} & $\mathbf{+0.118}$ & --- & \textbf{holds} \\
\bottomrule
\end{tabular}
\end{table}
