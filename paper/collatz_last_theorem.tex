...
\subsection{The empirical bound holds}
...
\begin{enumerate}
\item \textbf{Large deviations}: show that $P(D(\mathcal{O}) > c)$ decays with orbit length (the CLT route of Terras, repaired for the cluster structure).
...
\item \textbf{The 2-adic drift}: the exact drift $\log_2 a - 2 < 0$ for $a = 3$ is unconditional; the gap is promoting it from ensemble to per-orbit.
\end{enumerate}
